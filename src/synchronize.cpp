...
struct queue {
	queue(size_t max_capacity = 30) : max(max_capacity), closed(false), t0(AV_NOPTS_VALUE) {}
	...
	void enqueue(av::frame &f) {
		...
		std::cerr << (void*)this << std::dec
			  ...
			  << " enqueue frame: " << f.f->pts << " " << t0 + f.f->pts
			  << std::endl;
#endif
		filled.push_back(f);
	...
	bool acquire(int64_t pts, av::frame &f) {
		std::lock_guard<std::mutex> l(m);
		bool ret = false;

		pts -= t0;

		while (!filled.empty() && pts >= filled.front().f->pts ) {
			f = filled.front();
			filled.pop_front();
...
int main(int argc, char *argv[])
{
	...
	while (!q1.is_closed() && !q2.is_closed()) {
		...
		std::cerr << "f1: " << q1.t0 + f1.f->pts << std::endl;

//		f2 = q2.acquire();

//		std::cerr << "f2: " << q2.t0 + f2.f->pts << std::endl;

		// now search the "good" pts in q2
		if (q2.acquire(q1.t0 + f1.f->pts, f2)) {
			std::cerr << "got f2: " << q2.t0 + f2.f->pts << std::endl;
		}
	}
